...
O arquivo `server/update-cpf.sql` contém o script para atualizar CPF.
...
1. **Chave da API não configurada**: Verifique se a variável de ambiente `VITE_OPENAI_API_KEY` está configurada no arquivo `.env` na raiz do projeto.

2. **Formato da chave**: Certifique-se de que a chave da API está no formato correto (começa com `sk-`).
